...
import (
	...
	"dagger/infra/internal/querybuilder"
	"dagger/infra/internal/telemetry"
)
...
func (r Pipelines) MarshalJSON() ([]byte, error) {
	var concrete struct{}
	return json.Marshal(&concrete)
}

func (r *Pipelines) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Pipelines":
		...
		case "Release":
			...
			var dotenvKey *dagger.Secret
			if inputArgs["dotenvKey"] != nil {
				err = json.Unmarshal([]byte(inputArgs["dotenvKey"]), &dotenvKey)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg dotenvKey", err))
				}
			}
			return (*Pipelines).Release(&parent, ctx, source, dotenvKey)
		default:
